//! Watched port configuration for monitoring port status changes.
...
impl WatchedPort {
    ...
    /// Create a watched port from an existing ID (for deserialization/migration).
...
mod tests {
    ...
    fn test_serialization() {
        ...
        let deserialized: WatchedPort = serde_json::from_str(&json).unwrap();
        assert_eq!(wp.port, deserialized.port);
        assert_eq!(wp.notify_on_start, deserialized.notify_on_start);
        assert_eq!(wp.notify_on_stop, deserialized.notify_on_stop);
    }
    ...
    fn test_display() {
        ...
        let wp_start_only = WatchedPort::with_notifications(8080, true, false);
        assert_eq!(wp_start_only.to_string(), "Port 8080 (notify: start)");

        let wp_none = WatchedPort::with_notifications(9000, false, false);
        assert_eq!(wp_none.to_string(), "Port 9000 (no notifications)");
    }
}

...
/// - PortForwardRow: Kubernetes port-forward connection row
///
...
/// Menu bar row for Kubernetes port-forward connections
struct PortForwardRow: View {
    ...
    var body: some View {
        ...
        .contextMenu {
            Button {
                state.portForwardManager.restartConnection(connection.id)
            } label: {
            ...
            Button {
                if let url = URL(string: "http://localhost:" + String(connection.effectivePort)) {
            ...
            Button {
                ...
                NSPasteboard.general.setString("http://localhost:" + String(connection.effectivePort), forType: .string)
            } label: {
            ...
            Button(role: .destructive) {
                state.portForwardManager.removeConnection(connection.id)
            } label: {
